...
input_file = "puzzles/inputs/day8.txt"
...
# Calculate distance between two junction boxes
...
# Connect the first 1000 pairs by distance (whether or not they're already connected)
uf = UnionFind(n)
for idx in range(min(1000, len(pairs))):
    dist, i, j = pairs[idx]
    uf.union(i, j)

print(f"Processed first 1000 pairs")
...
if len(circuit_sizes) >= 3:
    ...
    print(f"Part 1: Product of three largest circuits: {result}")
else:
    print(f"Error: Only {len(circuit_sizes)} circuits found")

# Part 2: Continue connecting until all are in one circuit
uf2 = UnionFind(n)
...
for idx in range(len(pairs)):
    ...
    if uf2.union(i, j):
        ...
        circuit_sizes2 = uf2.get_circuit_sizes()
        if len(circuit_sizes2) == 1:
            break
...
if last_connected_pair:
    ...
    result2 = x1 * x2
    print(f"Part 2: Last connection was between boxes at X={x1} and X={x2}")
    print(f"Part 2: Product of X coordinates: {result2}")
else:
    print("Error: Could not connect all junction boxes into one circuit")
