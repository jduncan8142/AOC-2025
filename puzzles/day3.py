"""
--- Day 3: Lobby ---
You descend a short staircase, enter the surprisingly vast lobby, and are quickly cleared by
the security checkpoint. When you get to the main elevators, however, you discover that each
one has a red light above it: they're all offline.
...
"But, don't worry! It's not fried; it just needs power. Maybe you can get it running while I keep
...
In 987654321111111, you can make the largest joltage possible, 98, by turning on the first two
batteries.
In 811111111111119, you can make the largest joltage possible by turning on the batteries labeled
8 and 9, producing 89 jolts.
...
lose count of the number of times she needs to confirm "yes, I'm sure" and decorate the lobby 
...
In 234234234234278, the largest joltage can be found by turning on everything except a 2 battery, 
a 3 battery, and another 2 battery near the start to produce 434234234278.
...
input_file = "puzzles/inputs/day3.txt"

# Read the input file
with open(input_file, "r") as f:
    banks = [line.strip() for line in f.readlines()]

# Part 1: Find maximum joltage from each bank by selecting exactly two batteries
total_joltage = 0
for bank in banks:
    max_joltage = 0
...
# Part 2: Find maximum joltage by selecting exactly 12 batteries from each bank
def max_k_digits(s, k):
    """
    Find the largest k-digit number by selecting k digits from string s in order.
    Uses a greedy approach with a stack.
    """
    n = len(s)
    to_remove = n - k
    stack = []
    ...
    for i, digit in enumerate(s):
        # Remove smaller digits from the stack if we still have removals left
...
print(f"Part 2: Total output joltage: {total_joltage_part2}")
